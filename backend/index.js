...
const serviceMail = "darshanvirani010@gmail.com"
...
app.delete("/api/user", async (req, res) => {
    ...
    } else if (!await countDocuments(User, { email: email, isDelete: false })) {
        isValid = false;
    ...
    try {
        if (!adminId) {
            ...
            error.push(`admin with '${adminId}' id not exists`);
            isValid = false;
        }
    }catch(e){
        if(e.name === 'CastError'){
    ...
    const subject = "⚠️ Attention Required(Your Account is Suspended)"

    sendMail(serviceMail,email,subject,`Your Account is Deteted due to '${reason}'`)

    res.status(200).json({'msg':'User is Deleted'})
})

app.get("/api/users", async (req, res) => {
    const users = await find(User, { isDelete: false }, { firstName: 1, email: 1, ROLE: 1 });
...
app.post("/api/user/login", async (req, res) => {
    ...
    const userObj = await findOne(User, { email: body.email, password: body.password, isDelete: false });
    ...
    res.status(status).json({ 'mess': mess, 'userId': userId, 'ROLE': role});
})
...
app.get('/api/user', async (req, res) => {
    ...
    let userObj = await findOne(User, { _id: userId, isDelete: false });
...
app.get('/api/otp', async (req, res) => {
    ...
    sendMail(serviceMail, to, subject, "Your OTP is " + otpCode);
...
app.post('/api/send-mails', async (req, res) => {
    ...
    emails.forEach(e => sendMail(serviceMail, e, subject, message));
